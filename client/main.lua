...
function listApps()
    ...
    local width, height = term.getSize()
    local searchBarHeight = 3
    local maxSearchLength = width - menuWidth - 10
    local cursorPos = 1
    ...
    local function drawSearchBar()
        ...
        term.write(string.rep(" ", width - menuWidth - 10))
        term.setCursorPos(menuWidth + 3, 2)

        local displayQuery = searchQuery:sub(cursorPos, cursorPos + maxSearchLength - 2)
        term.write(displayQuery)

        if cursorVisible then
            term.setCursorPos(menuWidth + 2 + math.min(#displayQuery, #searchQuery) + 1, 2)
            term.setTextColor(colors.black)
            term.write("_")
        end
    ...
    local function drawApps()
        ...
        local response = http.get(server_addr .. "/search=" .. textutils.urlEncode(searchQuery))
        if response then
            ...
            if func then
                ...
                if apps and type(apps) == "table" then
                    ...
                    local rowHeight = 5
                    local rowsPerPage = math.floor((height - searchBarHeight - 1) / rowHeight)
                    local appsPerPage = rowsPerPage * columns + 3
                    local currentPage = 1
                    ...
                    local function drawPage()
                        ...
                        for index = startIndex, endIndex do
                            ...
                            local yStart = searchBarHeight + 1 + row * rowHeight
                    ...
                    while true do
                        ...
                        if event == "mouse_click" then
                            if x >= width - 7 and x <= width - 1 and y == 2 then
                                drawApps()
                            elseif x >= menuWidth + 2 and x <= menuWidth + 4 and y == height and currentPage > 1 then
                                currentPage = currentPage - 1
                                drawPage()
                            elseif x >= menuWidth + 4 + paginator_len and x <= menuWidth + width and y == height and currentPage < totalPages then
                                currentPage = currentPage + 1
                                drawPage()
                            elseif x <= menuWidth then
                                for i, btn in ipairs(buttons) do
                                    local btnY = 2 + (i - 1) * 2
                                    if y == 2 then
                                        listApps()
                                ...
                                for index = 1, appsPerPage do
                                    ...
                                    local column = (relativeIndex - 1) % columns
                                    local row = math.floor((relativeIndex - 1) / (columns * 3))
                                    local subRow = math.floor((relativeIndex - 1) % (columns * 3) / columns)

                                    local xStart = menuWidth + column * columnWidth + 2
                                    local yStart = searchBarHeight + 1 + row * (rowHeight * 3) + subRow * rowHeight
                        ...
                        elseif event == "char" then
                            if #searchQuery < maxSearchLength then
                                searchQuery = searchQuery .. button
                            else
                                cursorPos = cursorPos + 1
                            end
                            drawSearchBar()
                        elseif event == "key" and button == keys.backspace then
                            if #searchQuery > 0 then
                                searchQuery = searchQuery:sub(1, #searchQuery - 1)
                                if cursorPos > 1 then
                                    cursorPos = cursorPos - 1
                                end
                            end
